#ifndef SORT_LOAD_BALANCE_FUSE_REDUCTION_METHOD_H_
#define SORT_LOAD_BALANCE_FUSE_REDUCTION_METHOD_H_

#include "spmspv/config.h"

template<int NT, int VT, typename Type>
__device__ inline void DeviceRegToGlobalFuseAndReduction(
...
template<int NT, int VT, typename Type>
__device__ inline void DeviceRegToGlobalFuseAndReductionWriteBack(
         int bin_len, const int* reg_row,
         const Type* reg_val, int tid, 
         int* bin_row, Type* bin_val,
         Type* d_y) {
...
__global__ void KernelIntervalMoveIndirectFuseAndReduction(
...
template<typename Type>
__host__  inline void IntervalGatherIndirectFuseAndReduction(
  ...
  //  mgpu::my_counting_iterator<int>(0), bin_len, d_scan,
...
int LoadBalanceFuseExtractAndReductionDriver(
  ...
  first_timer.Start();
  IntervalGatherIndirectFuseAndReduction<vT>(bin_len, d_csc_col, 
                               d_ptr_col_len, x_nnz,  
                               d_csc_row, d_csc_val, 
