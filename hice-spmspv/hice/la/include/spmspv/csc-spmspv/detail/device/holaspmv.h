...
public:
	struct FlagSMem
	{
		...
		{
			if (ZERO_CHECK)
				return flags[1 + threadIdx.x / LM_WARP_SIZE];
			else
				return 0;
		}
...
private:
	...
	__device__ __forceinline__ uint32_t runSimple(SMem& smem, FlagSMem& flagmem, uint32_t blockRowStart, uint32_t blockNumRows, const OFFSET_TYPE* __restrict offsets)
	{
		...
		for (uint32_t r = threadIdx.x; r < blockNumRows; r += THREADS)
		{
			...
			if (ZERO_CHECK && ain == bin)
				atomicMax(&flagmem.flags[1 + max(0,a-1) / (LM_WARP_SIZE*NNZ_PER_THREAD)], r+2);
	...
	__device__ __forceinline__ void runThread(SMem& smem, FlagSMem& flagmem, uint32_t blockRowStart, uint32_t blockNumRows, const OFFSET_TYPE* __restrict offsets)
	{
		for (uint32_t r = threadIdx.x; r < blockNumRows; r += THREADS)
		{
			...
			if (ZERO_CHECK && ain == bin)
				flagmem.flags[1 + threada / LM_WARP_SIZE] = 1;
			else
			{
	...
	{
		for (uint32_t r = threadIdx.x / LM_WARP_SIZE; r < blockNumRows; r += THREADS / LM_WARP_SIZE)
		{
			int ain = smem.fetched_offsets[r];
			int bin = smem.fetched_offsets[r + 1];
			uint32_t a = max(0, ain);
			...
			else
			{
				...
				//iterate over all threads that start with that row
				for (uint32_t i = threada + laneid(); i <= threadb; i += LM_WARP_SIZE)
...
struct ValueLoader<THREADS, NNZ_PER_THREAD, false, VALUE_TYPE1, VALUE_TYPE2, VALUE_TYPE3, INDEX_TYPE, 0>
{
		...
		static void loadAndPremultiply(SMem& smem, VALUE_TYPE3(&values)[NNZ_PER_THREAD], uint32_t nnz, const VALUE_TYPE1* matrix, const INDEX_TYPE* inIndex, const VALUE_TYPE2* __restrict inVec)
	{
		...
		uint32_t warp_offset = blockIdx.x*(THREADS*NNZ_PER_THREAD) + threadIdx.x / LM_WARP_SIZE*LM_WARP_SIZE*NNZ_PER_THREAD;
		if (warp_offset + LM_WARP_SIZE*NNZ_PER_THREAD <= nnz)
		{
			//full warp load case
			INDEX_TYPE index_in[NNZ_PER_THREAD];
			warp_load_vectorized<VecSize>(index_in, inIndex + blockIdx.x*(THREADS*NNZ_PER_THREAD));
...
#pragma unroll
      ...
      uint32_t first_start = __shfl(my_row, 0);
			for (uint32_t r = first_start + laneid(); r < counterSmem.flags.zerorow() - 1; r += LM_WARP_SIZE)
			{
...
#pragma unroll
			for (int offset = 1; offset < LM_WARP_SIZE; offset *= 2)
			{
...
#pragma unroll
		for (uint32_t offset = 1; offset < LM_WARP_SIZE; offset *= 2)
		{
...
void hola_pre(size_t& tempmemsize, int m_rows, int m_cols, int m_nnz, int v_size, T* v_data/*,HolaMode mode*/) {
  ...
  HolaMode mode = HolaMode::Default;
  ...
  uint32_t requiredBlocks = hola_divup<uint32_t>(m_nnz, nnzperblock);
  //std::cout << "DEBUG: hola_pre: requiredblocks = " << requiredBlocks << std::endl;
...
//void hola_spmv(void* tempmem, size_t& tempmemsize, dDenseVector<T>& res, const dCSR<T>& m, const dDenseVector<T>& v, HolaMode mode, bool transpose, bool padded)
{
  ...
  bool transpose = false;
	...
	if (transpose && v_size != m_rows)
		throw std::runtime_error("SPMV dimensions mismatch");
	if (!transpose && v_size != m_cols)
		throw std::runtime_error("SPMV dimensions mismatch");
  ...
  HolaMode mode = HolaMode::Default;
	...
	// {
		if (transpose)
			mode = m_nnz < 300000 ? HolaMode::NonSortedFour : HolaMode::NonSortedEight;
		else
			mode = m_nnz < 300000 ? HolaMode::SortedFour : HolaMode::SortedEight;
	//}
	...
	// size_t outsize = transpose ? m_cols : m_rows;
	// if (res_size < outsize)
	...
	// 	if (nnzperthread == 4)
	// 		DetermineBlockStartsTranspose<unsigned int, 4 * HolaLaunchBounds<4, T>::Threads, T> <<< divup<uint32_t>(std::max(m_rows + 1, m_cols), HolaLaunchBounds<4, T>::Threads), HolaLaunchBounds<4, T>::Threads >> > (m_rows, m_cols, m_csr_row, reinterpret_cast<uint32_t*>(tempmem), res_data);
	// 	else
	// 		DetermineBlockStartsTranspose<unsigned int, 8 * HolaLaunchBounds<8, T>::Threads, T> <<<divup<uint32_t>(std::max(m_rows + 1, m_cols), HolaLaunchBounds<8, T>::Threads), HolaLaunchBounds<8, T>::Threads >> > (m_rows, m_cols, m_csr_row, reinterpret_cast<uint32_t*>(tempmem), res_data);
	...
	// 			break;
	// 		case HolaMode::NonSortedFour:
	// 			// (SORTED_RESOLVE)false, 
	//             // (BUFFERED_RESOLVE)false, 
	//             // (SORT_DIRECT)false, 
	//             // (ROWCOUNT_DIRECT_THRESHOLD)32, 
	...
	else
	{
		...
		else
		{
			...
			case HolaMode::SortedEight:
				...
				break;
			case HolaMode::NonSortedFour:
				MultiplyAlong<T, T, T, int, int, 4 * blockSize4, blockSize4, HolaLaunchBounds<4, T>::Bounds, false, ZeroCheck, Scale, false, 4 * blockSize4, 0><<<requiredBlocks, blockSize>>>(m_nnz, m_rows, m_cols, m_csr_val, m_csr_col, m_csr_row, v_data, res, 1, reinterpret_cast<uint32_t*>(tempmem));
				break;
			case HolaMode::NonSortedEight:
				MultiplyAlong<T, T, T, int, int, 8 * blockSize8, blockSize8, HolaLaunchBounds<8, T>::Bounds, false, ZeroCheck, Scale, false, 8 * blockSize8, 0><<<requiredBlocks, blockSize>>>(m_nnz, m_rows, m_cols, m_csr_val, m_csr_col, m_csr_row, v_data, res, 1, reinterpret_cast<uint32_t*>(tempmem));
				break;
			}
		}
...
//void hola_spmspv(void* tempmem, size_t& tempmemsize, dDenseVector<T>& res, const dCSR<T>& m, const dDenseVector<T>& v, const int* d_bit_vector, HolaMode mode, bool transpose, bool padded)
template<typename T>
                ...
                T* res, const int* d_bit_vector, /*HolaMode mode,*/ bool padded)
{
  ...
  HolaMode mode = HolaMode::Default;
	...
	// size_t outsize = transpose ? m_cols : m.rows;
	// if (res.size < outsize)
	// 	res.alloc(outsize + (padded ? 1024 : 0));
	// res.size = outsize;
	...
	else
	{
		...
		else
		{
			...
			case HolaMode::SortedEight:
				...
				break;
			case HolaMode::NonSortedFour:
				MultiplyAlongSpMspV<T, T, T, int, int, 4 * blockSize4, blockSize4, HolaLaunchBounds<4, T>::Bounds, false, ZeroCheck, Scale, false, 4 * blockSize4, 0><<<requiredBlocks, blockSize>>>(m_nnz, m_rows, m_cols, m_csr_val, m_csr_col, m_csr_row, v_data, d_bit_vector, res, 1, reinterpret_cast<uint32_t*>(tempmem));
				break;
			case HolaMode::NonSortedEight:
				MultiplyAlongSpMspV<T, T, T, int, int, 8 * blockSize8, blockSize8, HolaLaunchBounds<8, T>::Bounds, false, ZeroCheck, Scale, false, 8 * blockSize8, 0><<<requiredBlocks, blockSize>>>(m_nnz, m_rows, m_cols, m_csr_val, m_csr_col, m_csr_row, v_data, d_bit_vector, res, 1, reinterpret_cast<uint32_t*>(tempmem));
				break;
			}
		}
		err_r = cudaGetLastError();
...
// template void hola_spmspv(void* tempmem, size_t& tempmemsize, dDenseVector<float>& res, const dCSR<float>& m, const dDenseVector<float>& v, const int* d_bit_vector, HolaMode mode, bool transpose, bool padded);
// template void hola_spmspv(void* tempmem, size_t& tempmemsize, dDenseVector<double>& res, const dCSR<double>& m, const dDenseVector<double>& v, const int* d_bit_vector, HolaMode mode, bool transpose, bool padded);
