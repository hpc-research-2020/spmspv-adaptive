...
//added by 
__global__
void count_fast_track_num(unsigned int* csr5_partition_pointer, int len, int tail_start, int m) {
    int num = 0;
    for(int i=0; i<len-1; i++) {
        if(csr5_partition_pointer[i] == csr5_partition_pointer[i+1])
            num++;
    }
    printf("fast_track_tiles_num = %d, p = %d, num/(p-1) = %f\n", 
        num, len, num/(float)(len-1));
    printf("tail_start = %d, num_rows_last_tile = %d\n", tail_start, m-tail_start);
}
