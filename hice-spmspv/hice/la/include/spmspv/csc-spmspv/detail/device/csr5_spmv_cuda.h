                          ...
                          const vT            alpha)
{
    ...
    for (int i = 0; i < c_sigma; i++)
        sum += candidate<iT, vT>(d_value_partition, d_x, d_x_tex, 
        						d_column_index_partition, 
                    ...
                    const vT            alpha)
{
    ...
    //TODO:
    //All the threads read row_start from laneid 1.
    row_stop = __shfl(row_start, 1);
    //all the threads read row_start from laneid 0.
    row_start = __shfl(row_start, 0);
    //这个是为了：与31个1进行“&”操作，防止溢出访问。int最多32bit。
    row_stop &= 0x7FFFFFFF;
#else
