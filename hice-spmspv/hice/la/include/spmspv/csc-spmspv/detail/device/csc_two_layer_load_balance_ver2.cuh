...
// DeviceMemToMemLoopIndirect<NT>(x_nnz, csc_col, 
//     csc_row + range.z, tid, intervals_shared);

// template<int NT, typename InputIt, typename OutputIt, typename SourceIt>
// __device__ inline void DeviceMemToMemLoopIndirect(int count, InputIt source, 
...
/*****************************************/
// template<int NT, int VT>
// __device__ inline void DeviceGather(int count, InputIt data, 
//                              int indices[VT], 
//                              int tid, T* reg, bool sync) {
//
//DeviceGather<NT, VT, Type>(bin_len, csc_val, gather, tid, data);
...
__global__ void TwoLayerExtractKernel_shared(
  ...
  //                             interval, rank, gather);
  computeGather<Type, NT, VT>(nnz_per_block, blockRowStart, scan_shared, 
                              interval, rank, gather);
...
__host__  inline void TwoLayerLoadBalanceExtract(
  ...
  second_timer.Start();
  TwoLayerExtractKernel_shared<Type, THREADS, NNZ_PER_THREAD, NNZ_PER_BLOCK>
...
int TwoLayerLoadBalanceExtractDriver(
  ...
  TwoLayerLoadBalanceExtract<int, 4, 256, 1024>
