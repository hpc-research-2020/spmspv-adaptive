...
int Run(std::string file_name, std::string matrix_name, int m, int n, int mat_nnz,
  ...
  err = A.set_bitvector(d_bit_vector, num_ints);//指针传递
...
/*********select spmv.***********/
  ...
  if (csr5spmv_time/holaspmv_time > 1.0) {
    spmv_type = 0;
    A.set_spmv_type(0);//选择holaspmv
    A.CSR5Postprocess();
    std::cout << "DEBUG: select hola spmv\n";
  }else{
    spmv_type = 1;
    A.set_spmv_type(1);//选择csr5spmv
    A.holaPostprocess();
...
*/
  ...
  time = timer.Stop();
  std::cout << "naive col-spmspv time (sparse output) = " << time << " ms." << std::endl << std::endl;
  fout << time << " ";
...
#endif
    std::cout << "my lb col-spmspv time = " << time << "ms." << std::endl;
    fout << time << " ";
...
int doThis(std::string s_file_name, std::string dir_name) {
  ...
  std::string out_file = "/home/*/feature/" 
