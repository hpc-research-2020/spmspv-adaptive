...
//csc-based spmspv: vector-driven.
void SpmSpvCSCHost(int m, int n, int mat_nnz, 
                   int* csc_row, int* csc_col, VALUE_TYPE* csc_val, 
                   int x_nnz,  /*SparseVec* x,*/
                   int* sparse_x_key, VALUE_TYPE* sparse_x_val,
                   int* y_nnz, /*SparseVec* y,*/
                   int* sparse_y_key, VALUE_TYPE* sparse_y_val) {
  VALUE_TYPE* y_dense = (VALUE_TYPE* )malloc(sizeof(VALUE_TYPE) * m); 
  CHECK_MALLOC(y_dense);
  memset(y_dense, 0, sizeof(VALUE_TYPE) * m);

  int col_index, x_val;
  for (int i = 0; i < x_nnz; i++) {
    //col_index = x[i].key;
    //x_val = x[i].val;
    col_index = sparse_x_key[i];
    x_val = sparse_x_val[i];
    for (int j = csc_col[col_index]; j < csc_col[col_index + 1]; j++) {
      y_dense[csc_row[j]] += csc_val[j] * x_val;
    }
  }
  
  //convert y_dense to SparseVec.
  int index = 0;
  for (int i = 0; i < m; i++) {
#ifdef DOUBLE
      if (!IS_DOUBLE_ZERO(y_dense[i])) {
        // y[index].val = y_dense[i];
        // y[index].key = i;
        sparse_y_val[index] = y_dense[i];
        sparse_y_key[index] = i;
        index++;
      }   
#else
      if (!IS_FLOAT_ZERO(y_dense[i])) {
        // y[index].val = y_dense[i];
        // y[index].key = i;
        sparse_y_val[index] = y_dense[i];
        sparse_y_key[index] = i;
...
void TestSerialSpmspv(int m, int n, int mat_nnz,
                       int* csr_row, int* csr_col, VALUE_TYPE* csr_val,
                       int x_nnz,  /*SparseVec* x,*/
                       int* x_key, VALUE_TYPE* x_val, 
                       int* y_nnz,  /*SparseVec* y,*/
                       int* y_key, VALUE_TYPE* y_val, 
                       VALUE_TYPE alpha) {
  ...
  SpmspvTimer spmspv_ref_timer;
  spmspv_ref_timer.Start();

  int ref_iter = 1;
  for (int iter = 0; iter < ref_iter; iter++) {
     SpmSpvCSCHost(m, n, mat_nnz, 
                   csc_row, csc_col, csc_val, 
                   x_nnz, /*x,*/
                   x_key, x_val, 
                   y_nnz, /*y,*/
  ...
  double spmspv_ref_time = spmspv_ref_timer.Stop() / (double)ref_iter;
  std::cout << "spmspv sequential time = " << spmspv_ref_time 
            << "ms." << std::endl;
...
//WRONG!!
int IsColInxInBinarySearch(int target, int len, SparseVec* vec, 
                           int* vec_key){
  int left = 0;
  int right = len - 1;

  //don't use mid = (left+right)/2, 
  //because when left+right > max-INT, it will overflow.
  while (left <= right) {
    int mid = left + (right-left)/2;
    //if (vec[mid].key == target) {
    if (vec_key[mid] == target) {
      return mid;
    }
    else { 
      // if (vec[mid].key < target) {
...
int IsColInxInAllSearch(int target, int len, SparseVec* vec,
                        int* vec_key) {
  for (int i = 0; i < len; i++) {
    // if (target == vec[i].key){
    if (target == vec_key[i]){
...
int TestCscSortSpmspv(int m, int n, int mat_nnz,
                      int* csr_row, int* csr_col, 
                      VALUE_TYPE* csr_val,
                      int x_nnz, /*SparseVec* x,*/
                      int* x_key, VALUE_TYPE* x_val,
                      int* y_nnz, int* y_inx, VALUE_TYPE* y_val, 
                      VALUE_TYPE alpha) {
...
#if 1
  SpmspvHandle<int, unsigned int, VALUE_TYPE> A(m, n);
  //接收CSR的输入，存到结构体中，
  ...
  err = A.CscBasedSortSpmspv(alpha, y_nnz, y_inx, y_val, 1);

 //  double time = timer.Stop();
  // cout << "smsv cuda time = " << time << "ms." << endl;
  std::cout << "DEBUG: CscBasedSortSpmspv err = " 
            << err << std::endl;
  A.Destroy();
#endif
  return err;
...
int main(int argc, char** argv) {
  ...
  if (argc == 2) {
    ...
    std::cout << "Usage: matrix_file_name x_nnz, group_num" << std::endl;
    exit(1);
  }
  ...
  //#ifdef SMSV_SERIAL
    int y_nnz = 0;
    TestSerialSpmspv(m, n, mat_nnz,
                      csr_row, csr_col, csr_val,
                      x_nnz,  /*x_sparse,*/
                      x_sparse_key, x_sparse_val, 
                      &y_nnz,  /*y_sparse, */
                      y_sparse_key, y_sparse_val,
                      alpha);

    std::cout << "y_nnz= " << y_nnz << std::endl;
    SparseVec2DenseVec(y_dense, m, /*y_sparse,*/ y_sparse_key, y_sparse_val, y_nnz);
    Vec2File(y_dense, m, "../../res/spmspv_serial.vec");

    std::cout << "-------------------------------"  << std::endl;
  //#endif
   
  //#ifdef CSC_SORT_CUDA
    int* cu_y_sparse_inx = (int* )malloc(m * sizeof(int));
    VALUE_TYPE* cu_y_sparse_val = (VALUE_TYPE* )malloc(m * sizeof(VALUE_TYPE));
    CHECK_MALLOC(cu_y_sparse_inx);
    CHECK_MALLOC(cu_y_sparse_val);
    int cu_y_nnz = 0;
    TestCscSortSpmspv(m, n, mat_nnz, csr_row, csr_col, csr_val, 
                     x_nnz, /*x_sparse,*/ x_sparse_key, x_sparse_val, &cu_y_nnz, cu_y_sparse_inx, 
                     cu_y_sparse_val, alpha);
   
    std::cout << "cu_y_nnz= " << cu_y_nnz << std::endl;
    SparseVec2DenseVecSeparate(y_dense, m, cu_y_sparse_inx, cu_y_sparse_val, cu_y_nnz);
    Vec2File(y_dense, m, "../../res/spmspv_sort.vec");

    if (cu_y_sparse_inx) free(cu_y_sparse_inx);
    if (cu_y_sparse_val) free(cu_y_sparse_val);
    std::cout << "-------------------------------"  << std::endl;
  //#endif
  }
