...
global TEST
global BFS
ferr = open("generate_label-3.log", "a+")
def parser_info_data(fname):
  def get_label(csc_spmspv, spmv_s2d):
    c = [csc_spmspv, spmv_s2d]
  ...
  with open(fname) as fd, open("{}.dat".format(fname), "w+") as fout:
    ...
    #used_label_desc = "sort-naive-col, naive-col, sort-lb-col, lb-col".split(",")
    used_label_desc = "sort-naive-col, naive-col".split(",")
    #print(used_vec_desc)
...
def merge_raw_data(fname="raw_full.dat", _suffix=".info.dat"):
  dat_files = list_dir("test-model-3" if TEST else "data-model-3", suffix=_suffix)
...
if __name__ == '__main__':
  ...
  if TEST:
    ...
    BFS = True
    print("--test\n")
    conj_matrix_info_into_raw("test-model-3", _suffix=".info")
    merge_raw_data(fname="predict-model-3.dat",  _suffix=".info.dat")
  else:
    print("--train\n")
    conj_matrix_info_into_raw("data-model-3", _suffix=".info")
    merge_raw_data("raw_full-model-3.dat", ".info.dat")
