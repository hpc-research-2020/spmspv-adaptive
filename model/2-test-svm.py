...
def train_schedule_svm():
  ...
  #clf = SVC(C=10e5, gamma='auto',kernel='rbf',class_weight={0: 3, 1: 7})
  parameters = {'kernel':('rbf', 'rbf'), 'gamma':('auto', 'scale'), 'C':[1, 10000]}
  svc = SVC()
  ...
  save_model(clf, "schedule_svm-model-2.m")
...
def test_schedule(verbose=True):
  ...
  clf = load_model("schedule_svm-model-2.m")
