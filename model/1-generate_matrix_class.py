...
global TEST
global BFS
ferr = open("generate_label-1.log", "a+")
def parser_info_data(fname):
  def get_label(csc_spmspv, spmspv_s2a, spmv_s2d):
    c = [csc_spmspv, spmspv_s2a, spmv_s2d]
  ...
  with open(fname) as fd, open("{}.dat".format(fname), "w+") as fout:
    ...
    for line in lines[3:]:
      ...
      #new_data.append(get_label(line_data[label_idx[1]], line_data[label_idx[5]], line_data[label_idx[9]]))
      new_data.append(get_label(line_data[label_idx[1]], line_data[label_idx[4]], line_data[label_idx[8]]))
      #new_data.append(get_label_6(line_data[label_idx[0]],line_data[label_idx[1]],line_data[label_idx[3]], line_data[label_idx[5]], line_data[label_idx[7]], line_data[label_idx[9]]))
      #print(new_data)
...
def merge_raw_data(fname="raw_full.dat", _suffix=".info.dat"):
  dat_files = list_dir("test-model-1" if TEST else "data-model-1", suffix=_suffix)
...
if __name__ == '__main__':
  ...
  if TEST:
    ...
    BFS = True
    print("--test\n")
    conj_matrix_info_into_raw("test-model-1", _suffix=".info")
    merge_raw_data(fname="predict-model-1.dat",  _suffix=".info.dat")
  else:
    print("--train\n")
    conj_matrix_info_into_raw("data-model-1", _suffix=".info")
    merge_raw_data("raw_full-model-1.dat", ".info.dat")
