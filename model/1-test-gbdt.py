...
def train_schedule_dtree():
  ...
  ###parameters = {'criterion':('gini', 'entropy'), 'max_depth':[1, 10], 'class_weight':('balanced', '')}
  parameters = {'n_estimators':(50, 100, 200, 500), 'learning_rate':(0.1, 0.01), 'max_depth':[1,10]}
  ...
  save_model(clf, "schedule_gbdt-model-1.m")
...
def test_schedule(verbose=True):
  ...
  clf = load_model("schedule_gbdt-model-1.m")
